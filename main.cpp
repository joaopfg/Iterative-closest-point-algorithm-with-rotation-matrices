...
void ex2() {
  ...
  set_meshes(viewer);
    ...
    viewer.callback_pre_draw = [&](igl::opengl::glfw::Viewer & )->bool // run animation
    {
      ...
      cout << "Sum of the distances to nearest neghbors at iteration " << it << ": " << endl;
      float sum = 0.0;

      for(int i=0;i<V1.rows();i++)
        sum += sqrt((V1(i,0) - nn_V2(i,0))*(V1(i,0) - nn_V2(i,0)) + (V1(i,1) - nn_V2(i,1))*(V1(i,1) - nn_V2(i,1)) + (V1(i,2) - nn_V2(i,2))*(V1(i,2) - nn_V2(i,2)));
...
void ex3(){
  ...
  igl::opengl::glfw::Viewer viewer;
  viewer.callback_key_down = NULL; // for dealing with keyboard events
  viewer.data().add_points(V1,Eigen::RowVector3d(0.3, 0.8, 0.3));
  //set_pc(viewer);
  Eigen::MatrixXi I;
